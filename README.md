...
The drivers for the supported packages contain a gpio config file, which contains an extension for gpio control. These xacro files be found in `DRIVER/config/gpio_config.xacro` offer a template for the user to set the desiered GPIOs as for there usecase. The driver uses this list to export the command and state interfaces for the ROS2 Control. If there is no need for GPIO control, the `use_gpio` tag can be set to false so the gpio config file is not going to load.
